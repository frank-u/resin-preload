...
class BufferBackedWritableStream extends stream.Writable {
  constructor () {
    super(arguments)
    this.chunks = []
  }
...
const runContainer = (container) => {
  ...
  .then((stream_) => {
    docker.modem.demuxStream(stream_, process.stdout, process.stderr)
    return new Promise((resolve, reject) => {
      stream_.on('end', resolve)
...
preload.getDeviceTypeSlug = function (options) {
  ...
  return Promise.using(createContainerDisposer(options), (container) => {
    ...
    .then((stream_) => {
      ...
      docker.modem.demuxStream(stream_, out, new DevNull())
      return new Promise((resolve, reject) => {
        stream_.on('error', reject)
        stream_.on('end', () => {
